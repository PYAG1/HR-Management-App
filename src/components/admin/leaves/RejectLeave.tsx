...
export default function RejectLeave({ id }: { id: string | undefined }) {
  const [open, setOpen] = useState(false);
  const queryClient = useQueryClient();
  const cancelButtonRef = useRef(null);
  const { isLoading, isSuccess, mutate, isError, error } = useMutation({
    mutationFn: manageLeave,
    onSuccess: () => {
  ...
  useEffect(() => {
    ...
    if (isError) {
      toast.error(`Failed to reject, Error:${error}`);
    }
  }, [isSuccess, isError]);
                ...
                <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
                  ...
                  <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                    <button
                      ...
                      onClick={() => mutate({ id: id, approved: false })}
                    >
                      {isLoading ? (
